...
export default function OrderShipperDetail() {
  ...
  const orderDetail = orderList?.data?.filter(
  ...
  return (
    orderDetail[0] && (
      <div
        className=" mx-auto p-2 text-center md:w-96 relative h-screen "
        style={{ backgroundColor: "#e7edef" }}
      >
        <Box
          sx={{
            background: "white",
        ...
        >
          ...
          <Box className="text-right">
            <p>
              Giảm giá:{" "}
              {orderDetail[0].promotion
                ? (orderDetail[0].promotion.price * 1).toLocaleString()
                : 0}
              ₫
            </p>
